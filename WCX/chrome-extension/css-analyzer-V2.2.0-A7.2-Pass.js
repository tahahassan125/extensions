/**
 * WCX CSS Analyzer
 * V2.2.0-A7.2 - Inheritance Source Mapping
 *
 * Purpose:
...
(() => {
  ...
  function mapInheritanceSources(
    ...
    const winnersByElement =
      getWinningPropertyMapByElement(
    ...
    for (const detection of safeArray(detections)) {
      ...
      while (current) {
        ...
        const propertyMap =
          winnersByElement.get(current);
        const winner =
          propertyMap?.get(detection.property) || null;
        ...
        if (winner) {
          ...
          /*
           * CSS-wide keywords delegate, reset, or otherwise alter the
           * inheritance chain. They are not treated as the originating
           * source in A7.2; continue searching the ancestor chain.
           */
          if (!isCSSWideKeyword(sourceValue)) {
  ...
  function analyzeRenderingDependencies(
    ...
    /*
     * V2.2.0-A7.2: map each inherited property to the nearest ancestor
    ...
    return {
      ...
      summary: {
        ...
        /* V2.2.0-A7.2 inheritance source-mapping statistics. */
