...
(() => {
  ...
  function resolveVarReferencesForWinner(
    ...
    const references = parseVarFunctions(
      winner?.originalValue || winner?.value || "",
    );

    if (!references.length || !winner?.element) {
      return {
        status: references.length ? "unresolved" : "not-applicable",
        references: [],
        resolvedCount: 0,
        unresolvedCount: references.length,
      };
    }
    ...
    const resolvedReferences = references.map((reference) => {
      ...
      if (!source) {
        /*
         * V2.2-A4: if the referenced custom property is missing from
         * the applicable scope chain, resolve a literal fallback from
         * the same var() invocation. A fallback containing another
         * var() is intentionally deferred to V2.2-A5 (nested variables).
         */
        if (reference.fallback !== null) {
          ...
          if (fallbackReferences.length) {
            return {
              ...
              resolutionKind: "fallback-nested-pending",
              fallbackUsed: false,
              resolvedValue: null,
      ...
      return {
        ...reference,
        status: "resolved",
        resolutionKind: "custom-property",
        fallbackUsed: false,
        resolvedValue: source.winner.value || source.winner.originalValue || "",
        sourceTargetIndex: source.targetIndex,
        sourceTargetElement:
          source.winner.targetElement || null,
    ...
    const resolvedCount = resolvedReferences.filter(
      (reference) => reference.status === "resolved",
    ).length;

    const unresolvedCount =
      resolvedReferences.length - resolvedCount;
  ...
  function analyzeVarResolutions(
    ...
    /* V2.2-A4: literal fallback resolution is handled by
     * resolveVarReferencesForWinner(); nested fallback var()
     * chains remain pending for V2.2-A5. */
    const declarations = [];
    let resolvedCount = 0;
    ...
    for (const dependency of safeArray(renderingDependencies)) {
       ...
       * V2.2.0-A3.2
       *
       * Do not rely exclusively on the earlier declaration model's
       * variableReferences field. The source value is authoritative
       * for this stage, and parsing it here guarantees that a direct
       * var() introduced through an inline style or another source
       * declaration cannot be lost while records pass through the
       * cascade/expansion/deduplication pipeline.
       */
      const sourceValue =
      ...
      pendingCount += resolution.references.filter(
        (reference) => reference.status === "fallback-pending",
      ).length;

      declarations.push({
        elementLabel: dependency.elementLabel,
        property: dependency.property,
        value: dependency.originalValue || dependency.value || "",
  ...
  function analyzeRenderingDependencies(
    ...
    /*
     * V2.2-A3/A4: resolve direct var() references against the
     * winning custom-property definitions. V2.2-A4 additionally
     * resolves literal fallbacks when the referenced custom property
     * is missing. Recursive/nested variable resolution remains deferred
     * to V2.2-A5. Custom properties inherit through
